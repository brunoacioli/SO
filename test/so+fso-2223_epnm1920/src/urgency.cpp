...
void *thread_nurse(void *arg) {
   uint32_t id = *(uint32_t*)arg;

   printf("NURSE %d STARTED \n", id);
...
int main(int argc, char *argv[])
{
   ...
   for(uint32_t i = 0; i < npatients; i++) {
      patientID[i] = i;
      thread_create(&patient_thread[i], NULL, &thread_patient, &patientID[i]);
   }

   for(uint32_t i = 0; i < ndoctors; i++) {
      doctorID[i] = i;
      thread_create(&doctor_thread[i], NULL, &thread_doctor, &doctorID[i]);
   }

   for(uint32_t i = 0; i < nnurses; i++) {
      nurseID[i] = i;
      thread_create(&nurse_thread[i], NULL, &thread_nurse, &nurseID[i]);
   }
