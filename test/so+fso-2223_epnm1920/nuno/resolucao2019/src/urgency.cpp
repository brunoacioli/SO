...
void patient_goto_urgency(int id)
{
   ...
   printf("\e[32;01mPatient %s (number %u): get to hospital\e[0m\n", hd->all_patients[id].name, id);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   printf("%d done \n",id);
   check_valid_name(hd->all_patients[id].name);
   printf("\e[32;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);
...
void * nurse_thread(void * argv)
{
	while(1) 
      nurse_iteration();
}

void *doctor_thread(void * argv)
{
	while(1)
      doctor_iteration();
}
...
int main(int argc, char *argv[])
{
   ...
   uint32_t argID[npatients];	
   uint32_t argIDNurse[nnurses];	
   uint32_t argIDDoc[ndoctors];	
   for(uint32_t i = 0; i < npatients; i++)
   {
	   argID[i] = i;
	thread_create(&patients[i], NULL, patient_thread, &argID[i]);
   }
   
   for(uint32_t i = 0; i < nnurses; i++)
   {
	   argIDNurse[i] = i;
	thread_create(&nurses[i], NULL, nurse_thread, &argIDNurse[i]);
   }
   
   for(uint32_t i = 0; i < ndoctors; i++)
   {
	   argIDDoc[i] = i;
	   thread_create(&doctors[i], NULL, doctor_thread, &argIDDoc[i]);
   }
